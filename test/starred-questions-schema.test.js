...
const optimizationPath = new URL('../supabase/migrations/202609120002_optimize_starred_questions_rls.sql', import.meta.url);
...
test('starred question follow-up migration covers the foreign key and optimizes auth checks', async () => {
  const sql = await readFile(optimizationPath, 'utf8');
  assert.match(sql, /starred_questions_saved_source_idx/i);
  assert.match(sql, /on public\.starred_questions \(saved_source_id\)/i);
  assert.match(sql, /\(select auth\.uid\(\)\) = user_id/i);
  assert.match(sql, /source\.user_id = \(select auth\.uid\(\)\)/i);
});
